...
def get_url(url,p,message):
	...
	for x in soup.select('.xst'):
		get_msg(x['href'].split('-')[1],p,message)
...
for x in range(10):
	print('正在回复中...')
	get_url('http://www.miui.com/forum-'+choice(page)+'-'+str(x)+'.html',page,choice(arr))
	lis = list(range(30,120))
	time.sleep(choice(lis))